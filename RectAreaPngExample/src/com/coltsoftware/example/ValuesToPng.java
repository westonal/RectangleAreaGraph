...
public class ValuesToPng {
	...
	public static void main(String[] args) {
		RectangleSplit rectangleSplit = new RectangleSplit();
		...
		int width = 800;
		int height = 640;
		List<SplitResult> split = rectangleSplit.split(new Rectangle(0, 0,
				width - 1, height - 1));
		RenderedImage image = render(split, width, height);
		saveImage(image);
	}

	private static RenderedImage render(List<SplitResult> split, int width,
			int height) {
